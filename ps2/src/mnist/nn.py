...
def forward_prop(data, labels, params):
    ...
    N,P = data.shape
    K = labels.shape[1]
    a1 = sigmoid(np.dot(data,params["W1"])+params["b1"])
    a2 = softmax(np.dot(a1,params["W2"])+params["b2"])
    print("Out shape:", a1.shape)
    epsilon = 1e-32  # In some extreme cases, out = 0.
    loss = - np.multiply(labels, np.log(a2 + epsilon))
    loss = loss.sum() / N
    return a1, a2, loss
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    db2 = db2.reshape(1,K)
    print("shape",db2.shape,b2.shape)
